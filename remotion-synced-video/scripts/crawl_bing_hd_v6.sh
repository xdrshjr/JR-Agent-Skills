...
RESULT=$(agent-browser eval "$JS_CODE" 2>/dev/null)
echo "   提取到 URLs"
...
for url in data[:20]:
    print(url)
" > /tmp/img_urls.txt 2>/dev/null || echo "$RESULT" | grep -oE 'https://[^"\[\],]+' > /tmp/img_urls.txt
