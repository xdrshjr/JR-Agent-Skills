...
3. **BGM音量** - 设为解说音量的 30-50%（推荐 0.3）
4. **字幕显示** - `ttsText` 字段自动作为底部字幕，保持简洁易懂

## 字幕与背景音乐使用指南

### 📝 底部字幕

场景配置中的 `ttsText` 字段会自动显示为底部电影式字幕：

```json
{
  "id": "intro",
  "title": "人工智能",
  "ttsText": "人工智能，正在改变我们的世界。",
  "variant": "hero"
}
```
...
参数说明：
- `volume=0.3` - BGM 音量 30%（不盖过人声）
- `amix=inputs=2` - 混合两个音频
- `duration=first` - 以视频长度为准
- `-c:v copy` - 视频直接复制（快速处理）
4. **素材搜索** - 使用英文关键词效果最佳
